...
请注意在 `src/lib.rs` 中的那个 `pub use crate::front_of_house::hosting;` 语句同样不曾改变，同时那个 `use` 也不会对哪些文件作为代码箱的部分而被编译有任何的影响。这个 `mod` 关键字定义了模组，而 Rust 则会在于该模组有着同样名字的文件中，查找要进到那个模组中的代码。
